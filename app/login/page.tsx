...
import { Mail, Lock, LogIn, User, PenTool, ArrowLeft, Shield } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const [mode, setMode] = useState<'role-select' | 'publisher-login' | 'publisher-register' | 'reader-signup' | 'admin-login'>('role-select');
  const [selectedRole, setSelectedRole] = useState<'user' | 'publisher' | 'admin' | null>(null);
  ...
  const handlePublisherRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage('Publisher account requested! Please verify your email. Once verified, an admin will review and approve your account.');
      setFormData({ email: '', password: '', name: '', confirmPassword: '' });
    } catch {
      setError('Something went wrong. Please try again.');
    } finally {
      ...
      <div className={styles.loginContainer}>
        <div className={styles.loginContent}>
          ...
          {mode === 'publisher-login' && (
            <>
              <form className={styles.form} onSubmit={(e) => handleLogin(e, 'publisher')}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <div className={styles.inputWrapper}>
                    ...
                    <input type="email" name="email" placeholder="publisher@newsportal.com"
              ...
              <button
                ...
                onClick={() => { setMode('publisher-register'); setError(''); }}
              >
          ...
          {/* Admin Login */}
          {mode === 'admin-login' && (
            <form className={styles.form} onSubmit={(e) => handleLogin(e, 'admin')}>
              {error && <div className={styles.error}>{error}</div>}
              ...
              <div className={styles.formGroup}>
                ...
                <div className={styles.inputWrapper}>
                  <Mail size={20} className={styles.icon} />
                  <input type="email" name="email" placeholder="admin@newsportal.com"
                    value={formData.email} onChange={handleChange} />
                </div>
              </div>
              ...
              <div className={styles.formGroup}>
                ...
                <div className={styles.inputWrapper}>
                  <Lock size={20} className={styles.icon} />
                  <input type={showPassword ? 'text' : 'password'} name="password"
                    placeholder="Enter admin password" value={formData.password} onChange={handleChange} />
                  <button type="button" className={styles.showPassword}
                    onClick={() => setShowPassword(!showPassword)}>
                    {showPassword ? '👁️' : '👁️‍🗨️'}
                  </button>
            ...
            <>
              <div className={styles.socialSection}>
                ...
                <div className={styles.socialButtons}>
                  ...
                  <button type="button" className={styles.socialButton} onClick={() => handleSocialLogin('microsoft')}>
                    <FaMicrosoft size={20} /><span>Microsoft</span>
                  </button>
                  <button type="button" className={styles.socialButton} onClick={() => handleSocialLogin('apple')}>
                    <FaApple size={20} /><span>Apple</span>
                  </button>
                </div>
              </div>
          ...
          <p className={styles.footer}>
            ...
            {mode === 'publisher-login' && 'Sign in with your approved publisher credentials'}
            {mode === 'publisher-register' && 'Your account will be reviewed by an admin'}
            {mode === 'reader-signup' && 'Create a free account to read all articles'}
            {mode === 'admin-login' && 'Restricted access — authorized personnel only'}
          </p>
        </div>
      </div>
      <Footer />
